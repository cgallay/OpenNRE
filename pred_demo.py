...
pred_loader = nrekit.data_loader.json_file_data_loader(os.path.join(dataset_dir, 'pred.json'),
                                                       os.path.join(dataset_dir, 'word_vec.json'),
                                                       os.path.join(dataset_dir, 'rel2id.json'), 
                                                       mode=nrekit.data_loader.json_file_data_loader.MODE_ENTPAIR_BAG,
                                                       shuffle=False)

framework = nrekit.framework.re_framework(pred_loader, pred_loader)
...
res = framework.pred(model, ckpt_dir="checkpoint", model_name=dataset_name + "_" + model.encoder + "_" + model.selector, gpu_nums=1)
print(res)